...
if __name__ == '__main__':   
 for epoch in [EPOCHS]:
  ...
  frames=[i for i in range(len(com_list))]
  ...
  processes = [pool.apply_async(calc_dists, args=(x,data,)) for x in frames]
  results = [p.get() for p in processes]
  pool.close()
   
 

  for i_frame in range(len(results)):
         for i_dic in list(results[i_frame][0]):
              bl_p[i_dic] += results[i_frame][0][i_dic]
              bl_o[i_dic] += results[i_frame][3][i_dic]
         for i_dic in list(results[i_frame][1]):
              ba_p[i_dic] += results[i_frame][1][i_dic]
              ba_o[i_dic] += results[i_frame][4][i_dic]
         for i_dic in list(results[i_frame][2]):
              da_p[i_dic] += results[i_frame][2][i_dic]
              da_o[i_dic] += results[i_frame][5][i_dic]
  ...
  print(epoch,float(totalA_z),float(totalB_z),float(totalC_z),float(total_psi_z),file=open("./psi_z.dat","a+"))
